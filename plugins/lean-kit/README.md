...
### 3. Statusline v3 - 1-line compact statusline

Displays session info in a single line at the bottom. Auto-detects plan type; users can toggle display elements.

| Icon | Item | Dependency | Description |
|------|------|------------|-------------|
| 👤 | Account | `~/.claude.json` | Login email |
| 📁 | Directory | - | Current project path |
| 🌿 | Git branch | git | Current branch/commit |
| 🤖 | Model | - | Claude model |
| 🧠 | Context remaining | jq | With progress bar |
| 💰 | Cost + burn rate | jq | For API users ($/h) |
| 📋 | Plan type | `~/.claude.json` | Auto-detect Pro/Max/API |
| ⚡ | Extra Usage | `~/.claude.json` | Max plan only |
| ⌛ | Session remaining | jq + ccusage | Rate limit reset time |
...
**Activation by install method:**
- **Marketplace**: Run `/lean-kit:setup-statusline` (plan detection + element customization)
- **Standalone**: `install.sh` auto-registers in `settings.json`
...
`~/.claude/statusline.conf` controls display elements. Auto-generated by `/lean-kit:setup-statusline`.

```bash
# lean-kit statusline v3.0 config
# 0=hide, 1=show
SHOW_ACCOUNT=1       # 👤 Account
SHOW_DIR=1           # 📁 Directory
SHOW_GIT=1           # 🌿 Git
SHOW_MODEL=1         # 🤖 Model
SHOW_CONTEXT=1       # 🧠 Context
SHOW_COST=1          # 💰 Cost
SHOW_SESSION=1       # ⌛ Session
SHOW_PLAN=1          # 📋 Plan
SHOW_EXTRA_USAGE=1   # ⚡ Extra usage
PLAN_TYPE=           # Manual override: Pro | Max | API (empty = auto-detect)
```

Override config path via `STATUSLINE_CONF` env var:
...
| `STATUSLINE_CONF` | `~/.claude/statusline.conf` | Override statusline config path |
