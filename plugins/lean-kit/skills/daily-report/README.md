...
> 아래는 2026-02-25 리포트(`docs/daily-reports/2026-02-25/report.md`)에서 실제 발췌한 내용이다.
...
1. **모델 티어링**: 문서 작성/커밋 분리 같은 단순 작업은 Sonnet 4.6이나 Haiku를 활용하면 비용 절감이 가능하다 (현재 전 세션 Opus 4.6 사용).

2. **Context 관리**: 캐시 비용($248.17)이 전체의 90.6%를 차지한다. 세션 중간에 `/clear`로 불필요한 context를 정리하면 캐시 비용을 절감할 수 있다.

3. **추정 일일 예산 대비**: $273.93/일은 연간 약 $100K 수준이다. 프로젝트 수와 작업 강도를 고려한 예산 관리가 필요하다.
...
`~/.claude/projects/` 하위 JSONL 파일을 line-by-line 파싱, `assistant`/`user` 이벤트에서 토큰/도구/프롬프트를 추출한다.
...
- **KST 기준**: 모든 날짜/시간은 한국 표준시(UTC+9) 기준이다
- **최소 1개 세션 필요**: 해당 날짜에 세션 데이터가 없으면 리포트를 생성하지 않고 안내 메시지를 출력한다
- **Carry-over 세션**: 전날 시작~당일 종료한 세션은 해당일 리포트에 포함된다. 토큰/비용에 전날 분량이 포함될 수 있다
- **기존 리포트 덮어쓰기**: 같은 날짜의 리포트가 이미 있으면 덮어쓰기 여부를 확인한다
- **Node.js 필요**: 데이터 수집 스크립트 실행에 Node.js 런타임이 필요하다
- **로컬 전용**: 로컬 Claude Code 세션 데이터(`~/.claude/projects/`)를 기반으로 동작한다. CI/CD 또는 원격 환경에서는 세션 데이터가 없을 수 있다

## FAQ

| 질문 | 답변 |
|------|------|
| 세션이 없으면? | 데이터 수집 스크립트가 빈 결과를 반환하면, 오케스트레이터가 세션 수 0을 감지하여 안내 후 종료한다. |
| 비용은 얼마나? | Generator/Critic 모두 Sonnet 사용, 1회 실행당 ~$1–3 수준이다. |
| 캐리오버 세션이란? | 전날 시작되어 당일까지 이어진 세션이다. 해당일 리포트에 포함되며, 토큰/비용에 전날 분량이 포함된다. |
| 스크립트만 단독 실행 가능? | `node plugins/lean-kit/scripts/collect-session-data.mjs [YYYY-MM-DD]` 로 JSON 데이터만 추출할 수 있다. |
| 리포트 커스터마이징? | 생성된 `report.md`를 직접 편집할 수 있다. 리포트 템플릿 수정은 `references/report-template.md`를 편집하면 된다. |
| 이전 리포트와 비교? | Section 8에서 자동으로 이전 리포트와 트렌드를 비교한다. 이전 리포트가 없으면 베이스라인으로 기록한다. |
