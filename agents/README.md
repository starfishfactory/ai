...
### 🚀 Core Pack (3개) - 모든 사용자 추천
**"기본적인 개발 워크플로우에 필요한 핵심 agents"**

| 에이전트 | 목적 | 자동 선택 트리거 |
|----------|------|------------------|
| `molidae-core:code-reviewer` | 코드 가독성과 유지보수성 검토 | "코드 검토" |
| `molidae-core:test-generator` | TDD 테스트 케이스 생성 | "테스트 작성" |
| `molidae-core:korean-docs` | 한국어 기술 문서 작성 | "문서 작성" |

**추천 대상**: 모든 개발자

### ⚡ Advanced Pack (1개) - 특수 목적
**"프로젝트 관리가 필요할 때"**

| 에이전트 | 목적 | 사용 시점 |
|----------|------|-----------|
| `molidae-advanced:github-projects-manager` | GitHub Projects 칸반 보드 관리 | 프로젝트 관리 시 |

**추천 대상**: GitHub Projects를 활용하는 개발자
...
스크립트에서 제공하는 옵션:
- **🚀 Core Pack**: 모든 사용자 추천 (3개)
- **⚡ Advanced Pack**: Core + 특수 목적 (4개)
- **🛠️ Custom**: 개별 선택
...
```text
# 특정 에이전트 지정
> molidae-core:code-reviewer 에이전트로 이 코드를 검토해주세요
> molidae-core:test-generator 에이전트로 테스트를 작성해주세요
```
...
### v2.1.0 (2025-10-21)
- 🗑️ 중복 제거: 6개 → 4개 agents (기존 Claude Code agents와 충돌 최소화)
- ❌ 제거: debug-expert (root-cause-analyst 70% 중복), security-auditor (security-engineer 90% 중복)
- ♻️ 팩 재구성: Core 3개 + Advanced 1개
- ✨ 개선: 명확한 역할 분담, 한국어 특화 + 프로젝트 관리 강화
